...
function MqttClient (streamBuilder, options) {
  ...
  // Maximum first reconnect counter
  this.maxReconnectCounter = 0
  // maxFirstConnect Limit
  this.maxFirstConnect = this.options.maxFirstConnect!==undefined ?  this.options.maxFirstConnect  : null
  this.maxConnect = this.options.maxConnect !==undefined ? this.options.maxConnect : null
...
MqttClient.prototype._setupReconnect = function () {
  ...
  if (!that.disconnecting && !that.reconnectTimer && (that.options.reconnectPeriod > 0)) {
    ...
    that.reconnectTimer = setInterval(function () {
      that.maxReconnectCounter++
      if (that.maxFirstConnect !== null && that.maxReconnectCounter > that.maxFirstConnect) {
        that._clearReconnect()
        return that.emit('error', new Error('max first reconnect tries exceeded'))
      }
      if (that.maxConnect !== null && that.maxReconnectCounter > that.maxConnect) {
        that._clearReconnect()
        return that.emit('error', new Error('max reconnect tries exceeded'))
      }
